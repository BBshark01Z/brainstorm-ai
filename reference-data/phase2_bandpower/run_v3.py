#!/usr/bin/env python
"""Wrapper to run Phase 2C v3 and write output to a file."""
import sys
import os

out_path = os.path.join(os.path.dirname(__file__), "phase2c_v3_output.txt")
with open(out_path, "w", encoding="utf-8") as f:
    ...
    try:
        ...
        print("Modes:")
        print("  1) Welch n_per_seg=n_fft=full epoch (3000 samples)")
        print("  2) Welch n_per_seg=n_fft=256 (standard)\n")

        for mode_label, nps in [("Mode 1: Welch n_per_seg=n_fft=full epoch (3000)", epoch_samples), ("Mode 2: Welch n_per_seg=n_fft=256", 256)]:
            print(f"--- {mode_label} ---")
